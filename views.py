...
def upldcv(request):
    ...
    lid=request.POST['lid']
    save_image_path = r'C:\Users\athul\PycharmProjects\mockinterview\media/' + fs
...
def uploadresume(request):
    ...
    fh = open(r"C:\Users\athul\PycharmProjects\mockinterview\media\\" + timestr + ".pdf", "wb")
    path =timestr + ".pdf"
    fh.write(a)
    fh.close()
...
def voice(request):
        ...
        kk = "pic.jpg"
        pp=os.path.join(r'C:\Users\athul\PycharmProjects\mockinterview\media\emotion\\', reg)
        fn.save(os.path.join(r'C:\Users\athul\PycharmProjects\mockinterview\media\emotion\\', reg), im)
        ...
        fn.save(os.path.join(r'C:\Users\athul\PycharmProjects\mockinterview\media\audio', req),file)
        # fn.save(req,req)
        print(req, "++++++=====++++====+++")
        print("====ff1=====", ffl)
...
def silence_based_conversion(fl):
    path = r'C:\Users\athul\PycharmProjects\mockinterview\media/audio/'+fl+'.wav'
    print(path)
    r = sr.Recognizer()
    file=path
    # recognize the chunk
...
def and_view_emotion(request):
    lid=request.POST['lid']
    tid=request.POST['tid']
    print(lid)
    print(tid)
    res=answer_details.objects.filter(user__LOGIN_id=lid,vac_qn__vaccancy_id=tid)
    nu_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='neutral').count()
    ang_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='angry').count()
    d_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='disgust').count()
    fear_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='fear').count()
    happy_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='happy').count()
    sad_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='sad').count()
    surprise_count = answer_details.objects.filter(user__LOGIN_id=lid, vac_qn__vaccancy_id=tid, emot='surprise').count()
    data = []
    
    row = {"tips": 'neutral', "det": nu_count,"ang":'angry','ang_count':ang_count
           ,"dis":'disgust','d_count':d_count,"fear":'fear','fear_count':fear_count
        , "happy": 'happy', 'happy_count': happy_count,"sad": 'sad', 'sad_count': sad_count,"surprise": 'surprise', 'surprise_count': surprise_count
           }
    data.append(row)
    r = json.dumps(data)
    return HttpResponse(r)